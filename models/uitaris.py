...
def parse_string(input_string):
    try:
        # Regular expression to extract all coordinate pairs inside <|box_start|> and <|box_end|>
        match = re.search(r"<\|box_start\|>\((\d+),(\d+)\),\((\d+),(\d+)\)<\|box_end\|>", input_string)
        if match:
            x1, y1, x2, y2 = map(int, match.groups())  # Convert extracted values to integers
            return ((x1 + x2)/2, (y1 + y2)/2)  # Perform the required sum
        return None
...
class UiTarisModel():
    def load_model(self, model_name_or_path="bytedance-research/UI-TARS-7B-DPO"):
        ...
        self.generation_config = GenerationConfig.from_pretrained("bytedance-research/UI-TARS-7B-DPO", trust_remote_code=True).to_dict()
    ...
    def ground_only_positive(self, instruction, image):
        ...
        response1 = parse_string(response)
        print(response)
        result_dict = {
            "result": "positive",
            "format": "x1y1x2y2",
